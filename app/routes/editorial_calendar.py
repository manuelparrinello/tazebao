    ...
    while current_day <= grid_end:
        ...
        for _ in range(7):
            day_publications = sorted(
                ...
                key=lambda item: (item.publication_date, item.platform, item.content_type, item.title),
            )
            week.append(
                {
                    "date": current_day,
...
def apply_editorial_form(publication):
    ...
    publication.platform = request.form.get("platform") or "instagram"
    ...
    if publication.platform not in EDITORIAL_PLATFORMS:
...
def apply_publication_filters(query):
    ...
    if platform in EDITORIAL_PLATFORMS:
        query = query.filter(EditorialPublication.platform == platform)
